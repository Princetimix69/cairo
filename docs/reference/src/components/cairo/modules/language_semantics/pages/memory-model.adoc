...
fn main() {
    let x = MyStruct { a: 10_u128 };

    // Create a snapshot of x. `y` is of type `@MyStruct`.
    let y = @x;

    // Desnap `y` to get the value. This is allowed because MyStruct has the Copy trait.
    let z = *y;

    assert(x.a == z.a, 'Values should be equal');
}
----
